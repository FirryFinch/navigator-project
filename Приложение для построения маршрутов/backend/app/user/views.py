...
class AccessView(AuthRequiredMixin, CorsViewMixin, View):
    ...
    async def post(self):
        ...
        roles = [access.access_id.category_name for access in acceses]
        if (KEY_TYPES.ADMIN in roles) or (KEY_TYPES.OWNER in roles):
            ...
            return json_response(
                data={
                    "id": new_access.id,
                    "category_name": new_access.category_name,
                }
            )
        else:
            raise HTTPForbidden(reason="Доступ к ресурсу запрещен")
                    ...
                    "roles": [
                        {
                            "role": {
                                ...
                                "category_name": access_role.access_id.category_name,
                            },
                            "assignment_role_time": access_role.assignment_role_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                        }
            ...
            if (search_role.name == KEY_TYPES.OWNER) or (
                ...
                if new_access is None:
                    ...
                    return json_response(
                        data={
                            "id": new_access.id,
                            "role_name": new_access.role_name,
                            "access_id": new_access.access_id,
                            "user_id": new_access.user_id,
                            "assignment_role_time": new_access.assignment_role_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                        }
                    )
        else:
            new_access = await self.store.userAPI.create_access(
