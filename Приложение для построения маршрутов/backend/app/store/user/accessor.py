...
from app.user.models import UserModel, RoleModel, AccessClassModel
...
class UserAccessor(BaseAccessor):
    ...
    async def create_access(self, user_id: int, role_id: int) -> RoleDC | None:
        try:
            async with self.app.database.session() as session:
                useraccess = RoleModel(
                    user_id=user_id, access_id=role_id, assignment_role_time=datetime.datetime.utcnow()
                )
    ...
    async def create_access_class(self, category_name: str) -> AccessClassDC | None:
        try:
            async with self.app.database.session() as session:
                access = AccessClassModel(category_name=category_name)
                session.add(access)
                await session.commit()
                return access.to_dc()
        except sqlalchemy.exc.IntegrityError:
    ...
    async def get_user_accesses(self, user_id: int) -> RolesDC | None:
        try:
            async with self.app.database.session() as session:
                ...
                if user:
                    query = (
                        select(RoleModel)
                        .where(RoleModel.user_id == user_id)
                        .options(selectinload(RoleModel.accessgiven))
                    )
                    res = await session.scalars(query)
                    accesses = res.all()
                    return RolesDC(
                        RoleInfoDC(
                            ...
                            access_id=AccessClassDC(
                                id=role.accessgiven.id, category_name=role.accessgiven.category_name
                            ),
                            user_id=role.user_id,
                            role_name=role.role_name,
                            assignment_role_time=role.assignment_role_time,
                        )
